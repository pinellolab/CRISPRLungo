...
async function readFastqtoFasta(file) {
  // Transparently decompress gzip (.gz) input (extension or magic-byte detection).
...
self.onmessage = async function (event) {
  ...
  self.postMessage({ type: 0, fileLen: parseInt((fileLines.split('\n').length - 1) / 2) });
  ...
  var alignResDict = {};
  var checkSA = 1;
  var inputLineN = fileLines.length;
  ...
  while (checkSA > 0) {
    ...
    self.postMessage({ type: 1, wholeLen: inputLineN, alignedCount: parseInt(inputLineN / 4) - checkSA, n: n });
  }
